(function () {
  ...
  type KeycatWindow = Window &
    typeof globalThis & {
      KeycatVault: { init(options?: InitOptions): Promise<Provider> };
    };
  ...
  var ICON =
    "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='12' fill='%231f8a6b'/%3E%3Cpath d='M16 25 25 10l7 13 7-13 9 15v22H16z' fill='%23fffefa'/%3E%3Cpath d='M24 35h4v4h-4zm12 0h4v4h-4zm-11 12h14' stroke='%23182322' stroke-width='4' stroke-linecap='round'/%3E%3C/svg%3E";
  var state: State | null = null;
  var listeners: Record<string, Array<(...args: unknown[]) => void>> = {};
  var requestCounter = 0;

  function init(options?: InitOptions): Promise<Provider> {
    ...
    var pending: Record<string, Deferred> = {};
    ...
    function post(args: RequestArgs): Promise<unknown> {
    ...
    var provider: Provider = {
      request: function (args: RequestArgs) {
      ...
      removeListener: function (event: string, listener: (...args: unknown[]) => void) {
  ...
  function normalizeOptions(options: InitOptions): NormalizedOptions {
    ...
    var widgetUrl = new URL(
      options.widgetUrl || dataset.widgetUrl || DEFAULT_WIDGET_URL,
  ...
  function isInteractiveMethod(method: string) {
    return (
  ...
  function toProviderError(error: Record<string, unknown>) {
    return createError(
      typeof error.code === "number" ? error.code : -32603,
      typeof error.message === "string" ? error.message : "Keycat request failed.",
      error.data
    );
  }

  function createError(code: number, message: string, data?: unknown): ErrorWithCode {
    var error: ErrorWithCode = new Error(message);
  ...
  (window as KeycatWindow).KeycatVault = {
    init: init
  };
